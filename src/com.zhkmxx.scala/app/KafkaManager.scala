...
import com.zhkmxx.scala.app.KafkaCluster
import com.zhkmxx.scala.app.KafkaCluster.LeaderOffset
...
class KafkaManager(val kafkaParams: Map[String, String]) extends Serializable {
  ...
  private def setOrUpdateOffsets(topics: Set[String], groupId: String): Unit = {
    topics.foreach(topic => {
      ...
      if (partitionsE.isLeft) {
        println(s"[***********************************get kafka partition failed: ${partitionsE.left.get.mkString("\n")}")
      ...
      println("[***********************************consumerOffsetsE.isLeft: " + consumerOffsetsE.isLeft)
      log.info("consumerOffsetsE.isLeft: " + consumerOffsetsE.isLeft)
      if (hasConsumed) { // 消费过
        println("[***********************************consumerOffsetsE.isLeft: " + "消费过")
        log.warn("消费过")
        /**
  ...
  def updateZKOffsets(rdd: RDD[(String, String)]): Unit = {
    ...
    for (offsets <- offsetsList) {
      println("[***********************************offsets.topic " + offsets.topic)
      println("[***********************************offsets.untilOffset " + offsets.untilOffset)
      println("[***********************************offsets.partition " + offsets.partition)
